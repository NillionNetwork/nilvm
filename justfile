...
docker-build-builder-rust-gha:
    scripts/docker-build.sh builder-rust-gha "." --with-cache

docker-publish-builder-rust-gha: docker-build-builder-rust-gha
    scripts/docker-publish.sh --public x8g8t2h7 builder-rust-gha

test-wasm:
    cd wasm-workspace/nillion_client_wasm && wasm-pack test --node
...
build-dmg target:
    #!/usr/bin/env bash
    echo "Size of {{CARGO_TARGET_DIR}}/nillion-release/binaries/{{target}}:"
    du -sh "{{CARGO_TARGET_DIR}}/nillion-release/binaries/{{target}}"
    if command -v hdiutil > /dev/null;
    then
        hdiutil create -volname "nillion-sdk" -srcfolder "{{CARGO_TARGET_DIR}}/nillion-release/binaries/{{target}}" -ov -format UDZO "{{CARGO_TARGET_DIR}}/nillion-release/binaries/nillion-sdk-bins-{{target}}.dmg"
    else
        scripts/build-dmg.sh "{{CARGO_TARGET_DIR}}/nillion-release/binaries/nillion-sdk-bins-{{target}}.dmg" "{{CARGO_TARGET_DIR}}/nillion-release/binaries/{{target}}/"
    fi
