...
pub fn get_program(program_conf: &ProgramConf) -> Result<(JitProgram<MPCProtocol>, ProgramBytecode)> {
    MPCCompiler::compile_with_bytecode(get_program_mir(program_conf)?)
        .map_err(|e| eyre!("failed to compile program's MIR: {e}"))
}
...
impl Program {
    pub fn build(conf: &NadaProjectToml, program_name: &String) -> Result<Arc<Program>> {
        ...
        let program = Arc::new(Program { conf: program_conf.clone(), bytecode, program });
