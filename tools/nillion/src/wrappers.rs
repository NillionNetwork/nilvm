...
#[derive(Serialize)]
pub(crate) struct DisplayFriendlySecret(NadaValue<Clear>);

impl From<NadaValue<Clear>> for DisplayFriendlySecret {
    fn from(value: NadaValue<Clear>) -> Self {
        Self(value)
    }
}

impl fmt::Display for DisplayFriendlySecret {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match &self.0 {
            ...
            NadaValue::Array { values, .. } => {
                write!(f, "[")?;
                for value in values {
                    write!(f, "{}", DisplayFriendlySecret::from(value.clone()))?;
                }
                write!(f, "]")
            }
            NadaValue::Tuple { left, right } => {
                write!(
                    f,
                    "({},{})",
                    DisplayFriendlySecret::from(left.deref().clone()),
                    DisplayFriendlySecret::from(right.deref().clone())
                )
            }
            ...
            NadaValue::Object { values } => {
                ...
                for (key, value) in values {
                    write!(f, "{key}:{}", DisplayFriendlySecret::from(value.clone()))?;
                }
                write!(f, "]")
            }
            ...
            | NadaValue::EcdsaSignature(_) => Err(fmt::Error), // Unimplemented
        }
    }
}
