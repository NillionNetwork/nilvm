use crate::{
    args::{
        AddFundsArgs, AddIdentityArgs, AddNetworkArgs, BalanceCommand, Cli, Command, ComputeArgs, DeleteValuesArgs,
        EditIdentityArgs, EditNetworkArgs, IdentityGenArgs, OverwritePermissionsArgs, RemoveIdentityArgs,
        RemoveNetworkArgs, RetrievePermissionsArgs, RetrieveValuesArgs, ShowIdentityArgs, ShowNetworkArgs,
        StoreProgramArgs, StoreValuesArgs, UpdatePermissionsArgs, UseContextArgs,
    },
    context::ContextConfig,
    parse_input_file,
    serialize::{NoOutput, SerializeAsAny},
    wrappers::{
        ClusterInfo, DisplayFriendlySecret, PermissionsDelta, PreprocessingPoolStatus, UserFriendlyPermissions,
    },
};
...
use maplit::hashmap;
use nillion_client::{
    grpc::payments::AccountBalanceResponse, vm::VmClient, Ed25519SigningKey, Secp256k1SigningKey, TokenAmount, UserId,
};
use serde::Serialize;
use std::{
    collections::HashMap,
...
impl Runner {
    ...
    pub async fn run(&self, command: Command) -> Result<Box<dyn SerializeAsAny>> {
        match command {
            ...
            Command::PreprocessingPoolStatus => self.preprocessing_pool_status().await,
    ...
    pub async fn store_values(&self, args: StoreValuesArgs) -> Result<Box<dyn SerializeAsAny>> {
        ...
        let output = Box::new(hashmap! { "values_id" => values_id });

        Ok(output)
    }

    pub async fn retrieve_value(&self, args: RetrieveValuesArgs) -> Result<Box<dyn SerializeAsAny>> {
        let values = self.client.retrieve_values().values_id(args.values_id).build()?.invoke().await?;
        Ok(Box::new(values))
    }
    ...
    pub async fn retrieve_permissions(&self, args: RetrievePermissionsArgs) -> Result<Box<dyn SerializeAsAny>> {
        let permissions: UserFriendlyPermissions =
            (&self.client.retrieve_permissions().values_id(args.values_id).build()?.invoke().await?).into();
    ...
    pub async fn store_program(&self, args: StoreProgramArgs) -> Result<Box<dyn SerializeAsAny>> {
        ...
        let program_id = self.client.store_program().name(program_name).program(raw_mir).build()?.invoke().await?;
        let output = Box::new(hashmap! { "program_id" => program_id });
    ...
    pub async fn compute(&self, args: ComputeArgs) -> Result<Box<dyn SerializeAsAny>> {
        ...
        let compute_id = builder.build()?.invoke().await?;

        #[derive(Serialize)]
        struct ComputeResult {
            compute_id: String,
            #[serde(skip_serializing_if = "Option::is_none")]
            outputs: Option<HashMap<String, DisplayFriendlySecret>>,
        }
        ...
        let outputs = outputs
            .into_iter()
            .map(|(name, value)| (name, DisplayFriendlySecret::from(value.clone())))
            .collect::<HashMap<_, _>>();
    ...
    pub async fn preprocessing_pool_status(&self) -> Result<Box<dyn SerializeAsAny>> {
        let response = self.client.pool_status().invoke().await.context("querying preprocessing pool status")?;
    ...
    pub fn inspect_ids(&self) -> Result<Box<dyn SerializeAsAny>> {
        ...
        let output = Box::new(hashmap! { "user_id" => user_id.to_string() });
        Ok(output)
    }

    pub fn generate_key(seed: Option<String>, curve: &Kind) -> Result<SigningKey> {
    ...
    pub fn list_identities() -> Result<Box<dyn SerializeAsAny>> {
        ...
        let output = Box::new(hashmap! { "identities" => identities });
    ...
    pub fn show_identity(args: ShowIdentityArgs) -> Result<Box<dyn SerializeAsAny>> {
        ...
        let user_id = UserId::from_bytes(public_key.as_bytes());

        let output = Box::new(hashmap! {
    ...
    pub fn list_networks() -> Result<Box<dyn SerializeAsAny>> {
        ...
        let output = Box::new(hashmap! { "networks" => networks });
    ...
    pub fn show_network(args: ShowNetworkArgs) -> Result<Box<dyn SerializeAsAny>> {
        ...
        let mut output = hashmap! { "bootnode" => bootnode };
    ...
    pub fn show_context() -> Result<Box<dyn SerializeAsAny>> {
        let Some(context) = ContextConfig::load() else {
            return Ok(Box::<HashMap<String, String>>::new(hashmap! {}));
        };
