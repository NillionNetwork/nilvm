...
impl ProgramMIR {
    ...
    pub fn snippet(&self, source_ref: &SourceRef) -> std::result::Result<String, Error> {
        self.source_files
            .get(&source_ref.file)
            .map(|snippet| {
                let start_offset = source_ref.offset as usize;
                let end_offset = start_offset
                    .checked_add(source_ref.length as usize)
                    .ok_or_else(|| anyhow!("end offset overflow"))?;
                Ok(snippet.substring(start_offset, end_offset).trim().to_string())
            })
    ...
    pub fn source_info(&self, source_ref_index: SourceRefIndex) -> String {
        ...
        let Ok(snippet) = self.snippet(src_ref) else { return format!("{}:{}", src_ref.file, src_ref.lineno) };
        format!("{}  -> {}:{}", snippet, src_ref.file, src_ref.lineno)
    }
    ...
    pub fn text_repr(&self) -> String {
        ...
        for party in &self.parties {
            text.push_str(&format!("  {}   # {}\n", party.name, self.source_info(party.source_ref_index)));
        }
        ...
        for input in &self.inputs {
            text.push_str(&format!(
                ...
                input.party,
                input.doc,
                self.source_info(input.source_ref_index)
            ));
        }
        ...
        for output in &self.outputs {
            text.push_str(&format!(
                ...
                output.party,
                self.source_info(output.source_ref_index)
            ));
        }
        ...
        for function in &self.functions {
            ...
            text.push_str(&format!(
                ...
                self.source_info(function.source_ref_index)
            ));

            for operation in function.operations.values() {
                text.push_str(&format!("    {}", operation.text_repr(self)));
                text.push('\n');
            }
        }
        ...
        for operation in self.operations.values() {
            text.push_str(&operation.text_repr(self));
            text.push('\n');
        }
        text
    }
...
impl Operation {
    ...
    pub fn text_repr(&self, program_mir: &ProgramMIR) -> String {
        let mut op_str = delegate_to_inner!(self, text_repr);
        let snippet = program_mir.source_info(self.source_ref_index());
