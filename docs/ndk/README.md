...
The NDK consists of the onboarding material needed to launch a node on the Nillion Network.

* Read [Onboarding a Node](#onboarding-a-node) if you already have the infrastructure you need to launch a node

## Onboarding a Node

Read and submit the [questionnaire](./questionnaire.md) to begin onboarding.

### Networks

Choose one of the available networks listed below:

| Network Name    | Node/SDK Version | Node Configuration               |
| --------------- | ---------------- | -------------------------------- |
| nilvm-mainnet-1 | v0.9.0           | [node.yaml][node-yaml-mainnet-1] |
...
In addition, replace `payments.rpc_endpoint` with the dedicated, highly-available endpoint for
nilVM. Ask your Nillion contact for the URL.

As a reminder, the node listens on 2 network ports: `14311` (gRPC server) and `34111` (Prometheus
metrics) and need to be exposed appropriately.
