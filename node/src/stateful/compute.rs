...
use node_api::{
    auth::rust::UserId,
    compute::rust::{ComputeStreamMessage, OutputPartyBinding},
...
impl StateMachineIo for ExecutionVmIo {
    ...
    async fn open_party_stream(
        ...
        let initial_message =
            ComputeStreamMessage { compute_id: self.compute_id.as_bytes().to_vec(), bincode_message: vec![] };
...
impl StateMachineMessage<ComputeStreamMessage> for MPCExecutionVmMessage {
    ...
    fn encoded_bytes_as_output_message(message: Vec<u8>) -> ComputeStreamMessage {
        ComputeStreamMessage { compute_id: vec![], bincode_message: message }
    }
}
