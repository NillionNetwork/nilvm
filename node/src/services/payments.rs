...
use nillion_chain_client::tx::{PaymentTransactionRetriever, RetrieveError};
use node_api::{
    auth::rust::UserId,
    payments::rust::{
...
pub(crate) trait PaymentService: Send + Sync + 'static {
    ...
    fn config(&self) -> &PaymentsServiceConfig;
}
...
pub(crate) struct PaymentsServiceConfig {
    ...
    pub(crate) minimum_add_funds_payment: u64,
}
...
impl DefaultPaymentService {
    ...
    fn calculate_cost(&self, operation: &PriceQuoteRequest) -> QuoteFees {
    ...
    async fn fetch_transaction(&self, hash: &str) -> Result<Transaction, TransactionFetchError> {
        ...
        match result {
            ...
            Err(e) => Err(TransactionFetchError::Internal(e.to_string())),
        }
    }
    ...
    async fn do_generate_quote(&self, request: PriceQuoteRequest) -> Result<PriceQuote, QuoteError> {
        ...
        let fees = self.calculate_cost(&request);
...
impl PaymentService for DefaultPaymentService {
    ...
    async fn verify_payment(
        ...
        } else if tx.paid_amount < quote.fees.total {
            METRICS.inc_invalid_txs("insufficient_payment");
            warn!("Quoted {} but tx.paid_amount is {}", quote.fees.total, tx.paid_amount);
            return Err(PaymentVerificationError::InsufficientPayment);
        } else if tx.paid_amount > quote.fees.total {
            warn!("User paid more than expected: quote asked for {} but {} was paid", quote.fees.total, tx.paid_amount);
        }
    ...
    async fn deduct_payment_from_balance(
        ...
        let paid_amount = i64::try_from(quote.fees.total)
    ...
    async fn add_funds(&self, request: AddFundsRequest) -> Result<(), AddFundsError> {
        ...
        if transaction.paid_amount < self.config.minimum_add_funds_payment {
            return Err(AddFundsError::PaymentTooSmall);
        }
        // This should never fail but we want to avoid panic-able code.
        let paid_amount = i64::try_from(transaction.paid_amount)
            .map_err(|_| AddFundsError::Internal("funds overflow".to_string()))?;
        info!("Adding {paid_amount} unil to account {recipient}, paid for in tx {tx_hash}");
    ...
    fn default() -> Self {
        ...
        let invalid_transactions =
            Counter::new("invalid_tx_total", "Number of invalid nilchain transactions found by reason", &["reason"])
                .into();
        Self { operation_duration, invalid_transactions }
    }
}
...
mod test {
    ...
    async fn tx_nonce_mismatch() {
        ...
        let tx = PaymentTransaction {
            ...
            amount: TokenAmount::Unil(quote.fees.total),
        };
        ...
        assert_matches!(result, Err(PaymentVerificationError::NonceMismatch));
    }
    ...
    async fn reject_underpayment() {
        ...
        let tx = PaymentTransaction {
            ...
            from_address: "".to_string(),
            amount: TokenAmount::Unil(quote.fees.total - 1),
        };
        ...
        let result = service.verify_payment(quote, tx_hash).await;
        assert_matches!(result, Err(PaymentVerificationError::InsufficientPayment));
    }
    ...
    async fn quote_compute() {
        ...
        let service =
            ServiceBuilder { programs_service, auxiliary_material_metadata_service, ..Default::default() }.build();
    ...
    async fn generate_quote() {
        ...
        *cost.lock().unwrap() = quote.fees.total;
    ...
    async fn deduct_payment_from_balance() {
        ...
        let quote = PriceQuote {
            nonce: vec![1],
            fees: QuoteFees { total: 42, ..Default::default() },
            request: PriceQuoteRequest::PoolStatus,
            expires_at: Utc::now(),
    ...
    async fn add_funds() {
        ...
        builder
            .balance_repo
            .expect_add_funds()
            .with(eq(recipient), eq(amount.to_unil() as i64), always())
            .return_once(move |_, _, _| Ok(()));
        let expected_transfer =
            Transfer { tx_hash: request.tx_hash.clone(), account: recipient, amount: amount.to_unil() as i64 };
    ...
    async fn add_funds_small_payment() {
        ...
        let mut builder = ServiceBuilder::default();
        builder.config.minimum_add_funds_payment = 2;
        builder.tx_retriever.expect_get().with(eq(request.tx_hash.clone())).return_once(move |_| {
