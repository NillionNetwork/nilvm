...
impl proto::values_server::Values for ValuesApi {
    ...
    async fn retrieve_values(
        ...
        Ok(Response::new(RetrieveValuesResponse { values }.into_proto()))
    }
    ...
    async fn store_values(
        ...
        let user_id = request.user_id()?;
        let StoreValuesRequest { values, permissions, signed_receipt, update_identifier } =
...
impl From<UserValuesOperationError> for Status {
    fn from(e: UserValuesOperationError) -> Self {
        match e {
            ...
            UserValuesOperationError::NotFound => Status::not_found("value not found"),
