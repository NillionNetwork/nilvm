...
use crate::{
    ...
    stateful::{
        ...
        sm::{InitMessage, StateMachineArgs, StateMachineHandle, StateMachineRunner},
...
use node_api::{
    ...
    payments::rust::{
        InvokeComputeMetadata, OperationMetadata, Receipt, SelectedAuxiliaryMaterial, SelectedPreprocessingOffsets,
    },
...
impl ComputeApi {
    pub(crate) fn new(
        ...
        Self { our_party_id, channels, prime_builder, compute_handles, services, modulo }
    }

    fn transform_stream(
        peer: UserId,
        mut stream: Streaming<proto::stream::ComputeStreamMessage>,
    ) -> Receiver<MPCExecutionVmMessage> {
        let (tx, rx) = channel(STREAM_CHANNEL_SIZE);
        tokio::spawn(async move {
            while let Some(msg) = stream.next().await {
                match msg {
                    Ok(msg) => match MessageCodec.decode::<MPCExecutionVmMessage>(&msg.bincode_message) {
                        Ok(msg) => {
                            match tx.send(msg).await {
                                Ok(_) => continue,
                                Err(_) => {
                                    warn!(
                                        "Compute stream receiver was dropped while there were still messages queued up"
                                    );
                                }
                            };
                        }
                        Err(e) => {
                            warn!("Failed to decode VM message from {peer}: {e}");
                        }
                    },
                    Err(e) => {
        ...
        loop {
            ...
            let response = match result {
                OutputPartyResult::Success { values } => {
                    // this is temporary until clients are migrated
                    let bincode_values = nada_values_from_protobuf(values.clone(), &modulo)
                        .map_err(|_| "corrupted values")
                        .and_then(|v| MessageCodec.encode(&v).map_err(|_| "corrupted values"));
                    match bincode_values {
                        Ok(bincode_values) => RetrieveResultsResponse::Success { values, bincode_values },
                        Err(e) => RetrieveResultsResponse::Error { error: e.to_string() },
                    }
                }
...
impl proto::compute_server::Compute for ComputeApi {
    ...
    async fn invoke_compute(
        ...
        let InvokeComputeRequest { signed_receipt, value_ids, values, bincode_values, input_bindings, output_bindings } =
        ...
        // Insert an entry for this compute id if it doesn't exist yet. This could already be
        // created if another node received this message and sent us a `stream_compute` before we
        // received the client's RPC.
    ...
    async fn stream_compute(
        ...
        let stream = Self::transform_stream(user_id, stream);
    ...
    async fn retrieve_results(
        ...
        tokio::spawn(async move {
            if Self::wait_execution_result(results_service, compute_id, user_id, tx.clone(), modulo).await.is_err() {
