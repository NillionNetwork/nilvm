...
use crate::{
    ...
    services::payments::{
        AddFundsError, ConversionRateError, MinimumAddFundsPaymentError, PaymentService, PaymentVerificationError,
        QuoteError,
    },
};
...
impl proto::payments_server::Payments for PaymentsApi {
    ...
    async fn payments_config(
        ...
        let minimum_add_funds_payment = self.services.payments.minimum_add_funds_payment().await?;
        let credits_per_nil = self.services.payments.nil_credits_conversion_rate().await?;
        let response = proto::config::PaymentsConfigResponse {
            minimum_add_funds_payment: minimum_add_funds_payment.to_unil(),
            credits_per_nil,
        };
...
impl From<MinimumAddFundsPaymentError> for Status {
    fn from(e: MinimumAddFundsPaymentError) -> Status {
        error!("Failed to query minimum add funds payment: {e}");
        Status::internal("internal error")
    }
}

impl From<ConversionRateError> for Status {
    fn from(e: ConversionRateError) -> Status {
        error!("Failed to query nil/credits conversion rate: {e}");
        Status::internal("internal error")
    }
}
