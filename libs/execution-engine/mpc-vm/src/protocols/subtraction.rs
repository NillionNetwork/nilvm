//! Implementation of the MPC protocols for the subtraction operation
...
use jit_compiler::{
    ...
    models::{bytecode::Subtraction as BytecodeSubtraction, protocols::ExecutionLine},
    public_binary_protocol, share_binary_protocol,
};
use nada_value::{NadaPrimitiveType, NadaTypeMetadata};

binary_protocol!(Subtraction, "SUB", ExecutionLine::Local, RuntimeRequirementType);
into_mpc_protocol!(Subtraction);
impl Subtraction {
    public_binary_protocol!(BytecodeSubtraction);
    share_binary_protocol!(BytecodeSubtraction);

    /// Transforms a bytecode subtraction into a protocol
    pub(crate) fn transform<F: ProtocolFactory<MPCProtocol>>(
        context: &mut Bytecode2ProtocolContext<MPCProtocol, F>,
        operation: &BytecodeSubtraction,
...
pub(crate) mod vm {
    ...
    impl<T> Instruction<T> for Subtraction
    where
        T: SafePrime,
        ShamirSecretSharer<T>: SafePrimeSecretSharer<T>,
    {
            ...
            F: Instruction<T>,
        {
            ...
            match (&left_type, &right_type) {
                ...
                (left, right) => {
                    Err(anyhow!("unsupported operands for subtraction of shares protocol: {left:?} - {right:?}"))
                }
            }
        }
    }
