//! PREP-MOD2M protocol.
//!
//! This is the preprocessing protocol that speeds up the online phase in the MOD2M protocol.

use super::{PrepModulo2mShares, PrepModulo2mStateOutput};
use anyhow::anyhow;
...
pub mod states {
        ...
        ShamirSecretSharer<T>: SafePrimeSecretSharer<T>,
    {
        ...
        /// The shares produced by the PREP-COMPARE.
        pub(crate) prep_compare_shares: Batches<PrepCompareShares<T>>,
    }
}

/// The PREP-MOD2M protocol state.
#[derive(StateMachineState)]
#[state_machine(
    recipient_id = "PartyId",
    input_message = "PartyMessage<PrepModulo2mStateMessage>",
    output_message = "PrepModulo2mStateMessage",
    final_result = "PrepModulo2mStateOutput<PrepModulo2mShares<T>>",
    handle_message_fn = "Self::handle_message"
)]
pub enum PrepModulo2mState<T>
where
    T: SafePrime,
    ShamirSecretSharer<T>: SafePrimeSecretSharer<T>,
{
...
use PrepModulo2mState::*;

impl<T> PrepModulo2mState<T>
where
    T: SafePrime,
    ShamirSecretSharer<T>: SafePrimeSecretSharer<T>,
{
    /// Construct a new PREP-MOD2M state.
    pub fn new(
        element_count: usize,
        kappa: usize,
        k: usize,
        secret_sharer: Arc<ShamirSecretSharer<T>>,
    ) -> Result<(Self, Vec<StateMachineMessage<Self>>), PrepModulo2mCreateError> {
        // Check sizes
        let prime_length = T::MODULO.bits();
        if k.checked_add(kappa).ok_or(PrepModulo2mCreateError::IntegerOverflow)? >= prime_length {
            return Err(PrepModulo2mCreateError::StatisticalAndMaxSecretLargeForFieldSize);
        }

        let batch_size_bits = k.checked_add(kappa).ok_or(PrepModulo2mCreateError::IntegerOverflow)?;

        let batch_size_prep_compare = 1;
        let total_prep_compare =
            element_count.checked_mul(batch_size_prep_compare).ok_or(PrepModulo2mCreateError::IntegerOverflow)?;
        ...
        let messages =
            messages.into_iter().map(|message| message.wrap(&PrepModulo2mStateMessage::RanBitwise)).collect();
    ...
    fn transition_waiting_ran_bit(state: states::WaitingRanBitwise<T>) -> StateMachineStateResult<Self> {
        ...
        let messages =
            messages.into_iter().map(|message| message.wrap(&PrepModulo2mStateMessage::PrepCompare)).collect();
    ...
    fn transition_waiting_compare(state: states::WaitingCompare<T>) -> StateMachineStateResult<Self> {
        ...
        for (bit, prep_compare) in zipped {
            let share = PrepModulo2mShares { ran_bits_r: bit, prep_compare };
            shares.push(share);
        }
        let output = PrepModulo2mStateOutput::Success { shares };
        Ok(StateMachineStateOutput::Final(output))
    }

    fn handle_message(
        mut state: Self,
        message: PartyMessage<PrepModulo2mStateMessage>,
    ) -> StateMachineStateResult<Self> {
        use PrepModulo2mStateMessage::*;
        ...
        match (message, &mut state) {
            (RanBitwise(message), WaitingRanBitwise(inner)) => {
                match inner.ran_bit_state_machine.handle_message(PartyMessage::new(party_id, message))? {
                    ...
                    StateMachineOutput::Final(_) => {
                        Ok(StateMachineStateOutput::Final(PrepModulo2mStateOutput::RanAbort))
                    }
                    output => state.wrap_message(output, PrepModulo2mStateMessage::RanBitwise),
                }
            }
            (PrepCompare(message), WaitingCompare(inner)) => {
                match inner.prep_compare_state_machine.handle_message(PartyMessage::new(party_id, message))? {
                    ...
                    StateMachineOutput::Final(_) => {
                        Ok(StateMachineStateOutput::Final(PrepModulo2mStateOutput::PrepCompareAbort))
                    }
                    output => state.wrap_message(output, PrepModulo2mStateMessage::PrepCompare),
                }
            }
...
/// A message for the PREP-MOD2M protocol.
#[derive(Clone, Debug, Serialize, Deserialize, PartialEq)]
#[repr(u8)]
pub enum PrepModulo2mStateMessage {
...
pub enum PrepModulo2mCreateError {
    /// An integer overflow.
