...
use super::super::offline::{state::PrepModuloCreateError, PrepModuloShares};
use crate::{
...
pub struct PrepModuloValidator<T>(PhantomData<T>);

/// Default implementation for SafePrimes.
/// #[derive(Default)] doesn't work because [`SafePrime`] does not implement [`Default`]
impl<T: SafePrime> Default for PrepModuloValidator<T> {
    fn default() -> Self {
        Self(Default::default())
    }
}

impl<T: SafePrime> PrepModuloValidator<T> {
    /// Validates the output of a single PREP-MODULO run.
    fn validate_single(
        &self,
        kappa: usize,
        k: usize,
        party_shares: HashMap<PartyId, PrepModuloShares<T>>,
    ) -> Result<(), Error> {
        let mapper = PartyMapper::<PrimeField<T>>::new(party_shares.keys().cloned().collect())?;
        let kappa_plus_k = kappa.checked_add(k).ok_or(PrepModuloCreateError::IntegerOverflow)?;
        ...
        validator.validate(2, prep_comp_hashmap).expect("validation failed");
    ...
    pub fn validate(
        ...
        kappa: usize,
        k: usize,
        party_shares: HashMap<PartyId, Vec<PrepModuloShares<T>>>,
    ) -> Result<(), Error> {
        let mut party_single_share: Vec<HashMap<PartyId, PrepModuloShares<T>>> =
...
/// Builder that creates PREP-MODULO shares without running PREP-MODULO.
///
/// **This is meant to be used for testing purposes only**.
pub struct PrepModuloSharesBuilder<'a, T: SafePrime> {
    secret_sharer: &'a ShamirSecretSharer<T>,
    k: usize,
    kappa: usize,
}

impl<'a, T> PrepModuloSharesBuilder<'a, T>
where
    T: SafePrime,
    ShamirSecretSharer<T>: SafePrimeSecretSharer<T>,
{
    ...
    /// Build `count` PREP-MODULO shares.
    pub fn build(mut self, count: usize) -> Result<PartyShares<Vec<PrepModuloShares<T>>>, Error> {
        let mut party_shares: PartyShares<Vec<PrepModuloShares<T>>> = PartyShares::default();
        for _ in 0..count {
    ...
    fn build_one(&mut self) -> Result<PartyShares<PrepModuloShares<T>>, Error> {
    ...
    fn share_values(&self, ran_bits_r: Vec<ModularNumber<T>>) -> Result<PartyShares<PrepModuloShares<T>>, Error> {
        let builder = PrepCompareSharesBuilder::new(self.secret_sharer, rand::thread_rng())?;
        let mut prep_compare = builder.build(2).expect("failed to build PREP-COMPARE shares");
        ...
        for (party_id, ran_bits_r) in ran_bits_r {
            ...
            output_shares.insert(party_id, PrepModuloShares { ran_bits_r, prep_compare });
        }
        Ok(output_shares)
    }
}
