...
use super::state::{SecretBitAdderOperands, SecretBitAdderState};
use crate::{
...
struct SecretBitAdderProtocol<T: SafePrime> {
    operands: Vec<(u64, u64)>,
    polynomial_degree: u64,
    _unused: PhantomData<T>,
}

impl<T: SafePrime> SecretBitAdderProtocol<T> {
...
impl<T: SafePrime> Protocol for SecretBitAdderProtocol<T>
where
    T: SafePrime,
    ShamirSecretSharer<T>: FieldSecretSharer<PrimeField<T>> + FieldSecretSharer<PrimeField<T::SophiePrime>>,
{
    type State = SecretBitAdderState<T>;
    type PrepareOutput = SecretBitAdderConfig<T>;
    ...
    fn prepare(&self, parties: &[PartyId]) -> Result<Self::PrepareOutput, Error> {
        ...
        let mut party_operands: PartyShares<Vec<SecretBitAdderOperands<T>>> = PartyShares::default();
        for (left, right) in self.operands.iter() {
            ...
            for (party_id, left_bits) in left_party_bits.into_iter() {
                ...
                let operand = SecretBitAdderOperands::new(left_bits.into(), right_bits.clone().into());
                party_operands.entry(party_id.clone()).or_default().push(operand);
            }
        }
        Ok(SecretBitAdderConfig { parties: parties.to_vec(), party_operands })
    }
    ...
    fn initialize(
        ...
        let (state, messages) = SecretBitAdderState::new(operands, Arc::new(secret_sharer))?;
...
struct SecretBitAdderConfig<T: Modular> {
    parties: Vec<PartyId>,
    party_operands: PartyShares<Vec<SecretBitAdderOperands<T>>>,
}
    ...
    ShamirSecretSharer<T>: FieldSecretSharer<PrimeField<T>> + FieldSecretSharer<PrimeField<T::SophiePrime>>,
{
    ...
    let protocol = SecretBitAdderProtocol::<T>::new(inputs, polynomial_degree);
