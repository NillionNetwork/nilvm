    ...
    O: PaidVmOperation,
{
    ...
    pub async fn quote(self) -> Result<PaidOperation<'a, O, QuotedState>, QuoteError> {
        ...
        let signed_quote = retrier.invoke_single(PaymentsClient::price_quote).await?;
    ...
    pub async fn pay(self) -> Result<PaidOperation<'a, O, PaidState>, PaymentError> {
        ...
        let state = match state.payment_mode {
            ...
            PaymentMode::PayPerOperation => {
                ...
                let tx_hash = client
                    .nilchain_payer
                    .submit_payment(state.quote.fees.tokens, state.quote.nonce.clone())
                    .await
                    .map_err(|e| PaymentError(e.to_string()))?;
                info!("Payment of {} made in {:?}", TokenAmount::Unil(state.quote.fees.tokens), start_time.elapsed());
    ...
    O: PaidVmOperation,
{
    ...
    pub async fn validate(self) -> Result<PaidOperation<'a, O, ValidatedState>, ReceiptError> {
        ...
        let signed_receipt = match result {
            ...
            Err(e) => return Err(ReceiptError::new("failed to get payment receipt", e)),
        };
...
trait FromStatusDetailed: From<Status> {
    fn new(reason: &str, e: Status) -> Self {
        let (message, code) = (e.message(), e.code());
        Status::new(code, format!("{reason}: {message}")).into()
    }
}

impl<T: From<Status>> FromStatusDetailed for T {}
